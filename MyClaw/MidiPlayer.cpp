...
MidiPlayer::MidiEvent MidiPlayer::get_next_event() const
{
...
bool MidiPlayer::is_track_end(const MidiEvent& e)
{
	if (e.event == 0xFF && e.data[1] == 0x2F)
		return true;
	return false;
}
...
void MidiPlayer::print_event(const MidiEvent& evt)
{
	uint8_t event = evt.event & 0xF0;
	uint8_t channel = evt.event & 0x0F;
	uint8_t data1 = evt.data[1], data2 = 0;
